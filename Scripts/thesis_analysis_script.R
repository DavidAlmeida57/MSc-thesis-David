...
#Note: On the Venn Digram's it's better if you run the code indivually, otherwise the plots are cleaned before
# the next plots,so they don't stay in the plot history, it's like this because they overlay the previous plots
...
# --- Input file paths ---
input_excel_path <- file.path(base_dir, "Data", "Results_amostras.xlsx")
biogrid_path     <- file.path(base_dir, "Data", "BIOGRID-GENE-106848-4.4.246.tab3.txt")

# --- Output file paths ---
out_pre_filtering <- file.path(base_dir, "Data", 
                               "Dados Proteinas antes da filtragem.xlsx")
out_groups_list   <- file.path(base_dir, "Data", 
